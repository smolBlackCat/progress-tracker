...
ui::CardWidget::CardWidget(std::shared_ptr<Card> card_refptr)
    ...
    add_option("remove", "Remove", sigc::mem_fun(*this, &ui::CardWidget::remove));
}
...
ui::Cardlist const* ui::CardWidget::get_cardlist() {
    return cardlist_p;
}

std::shared_ptr<Card> ui::CardWidget::get_card() {
    return card_refptr;
}

bool ui::CardWidget::equal(ui::CardWidget& other) {
    return cardlist_p == other.get_cardlist();
}
