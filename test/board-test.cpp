...
TEST_CASE("Board Instatiation", "[Board]") {
    auto board = Board::create("Name", "rgba(12,12,12,0.7)");
...
TEST_CASE("Name Changing", "[Board]") {
    auto target_board = Board::create("GTK Programming", "rgb(12,12,12)");
...
TEST_CASE("Colour Setting", "[Board]") {
    auto target_board = Board::create("Custom Board", "rgb(12,12,12)");
...
TEST_CASE("Signal Emission", "[Board]") {
    auto target_board = Board::create("Simple Name", "rgba(12,23,18,0.4)");
...
TEST_CASE("Modification State", "[Board]") {
    auto target_board = Board::create("Name", "rgba(1,1,1,0.1)");
    ...
    SECTION("Modify after container alteration") {
        auto cardlist = CardList::create("Something else");
        target_board->container().append(cardlist);
        REQUIRE(target_board->modified());
    }
}